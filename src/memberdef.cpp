...
static bool writeDefArgumentList(OutputList &ol,const Definition *scope,const MemberDef *md)
{
  ...
  ol.endParameterName(TRUE,nArgs<2,!md->isObjCMethod());
  ol.popGeneratorState();
                      ...
                      bool onlyText) const
{
  ...
  static const reg::Ex operatorRegex(R"^operator[\\=\\+\\-\\*\\/\\%\\=\\!\\>\\<\\|\\^\\~\\[]");
  if (search(n, operatorRegex))
    n = n.insert(8, ' ');
                                  ...
                                  ) const
{
  ...
  const ArgumentList &templateDocArgList = m_impl->templateMaster ?
                                   ...
                                   m_impl->tArgList;
  if (!templateDocArgList.isEmpty() && templateDocArgList.hasTemplateDocumentation() && !templateDocArgList.allHidden())
  {
    QCString paramDocs;
