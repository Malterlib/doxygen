  ...
  public:
   ...
   ~ClassDefImpl();
    ...
    virtual QCString qualifiedNameWithTemplateParameters(
        const ArgumentLists *actualParams=0,uint *actualParamIndex=0,bool forDisplay=true) const;
...
class ClassDefAliasImpl : public DefinitionAliasMixin<ClassDef>
{
  public:
    ...
    virtual QCString qualifiedNameWithTemplateParameters(
        const ArgumentLists *actualParams=0,uint *actualParamIndex=0,bool forDisplay=false) const
    { return makeQualifiedNameWithTemplateParameters(this,actualParams,actualParamIndex,forDisplay); }
    virtual bool isAbstract() const
...
void ClassDefImpl::writeDetailedDocumentationBody(OutputList &ol) const
{
  ...
  ArgumentList const &templateDocArgList = templateMaster() ? templateMaster()->templateArguments() : templateArguments();
  
  if (templateDocArgList.hasTemplateDocumentation() && !templateDocArgList.allHidden())
  {
    QCString paramDocs;
    // convert the parameter documentation into a list of @tparam commands
    for (auto const &a : templateDocArgList)
    {
    ...
    ol.generateDoc(docFile(),docLine(),this,0,paramDocs,TRUE,FALSE,
                   QCString(),FALSE,FALSE,Config_getBool(MARKDOWN_SUPPORT));
  }
